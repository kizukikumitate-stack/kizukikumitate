...
DATA = [
 ...
 dict(
  ...
  dict(
  slug='jirei-tatewari', no='No.008', cat='so',
  title='縦割りの解消',
  subtitle='仕組みで橋を架けた会社と、交流イベントに頼った会社',
  win_head='成功：評価と場の仕組みごと変えた会社（マイクロソフト、トヨタ）',
  win=['マイクロソフトは社員どうしを競わせる相対評価を2013年に廃止。ナデラCEOのもと「他の人の成功への貢献」を評価に組み込んだ。',
       '協力が損になる構造をやめてから文化が動き、10年で株式時価総額はおよそ10倍に。',
       'トヨタの「大部屋」方式は、部門の関係者が同じ部屋で同じ情報を見て決める場の設計。初代プリウスは正式着手から約2年で量産化。',
       '共通するのは、仲良くなることを目的にせず「共通の目標・同じ情報・協力が報われる評価」を設計したこと。'],
  lose_head='失敗：交流イベントに頼った会社（典型パターン）',
  lose=['チャット導入・フリーアドレス・シャッフルランチ・懇親会。雰囲気は和やかになった。',
        '半年後、席は毎日同じ島に固まり、横断チャンネルは告知だけ。案件は今日も「持ち帰って検討します」。',
        '横断プロジェクトは兼務者に丸投げされ、本業の合間の作業になり静かに止まった。',
        '各部門は目標・予算・人員を守っただけ。協力が損になる構造の上では、懇親会を重ねても翌朝の会議は変わらない。'],
  fork='部門の壁に仕組みの橋を架けたか、交流の場だけを増やしたか。',
  questions=['いまの施策は交流の「量」を増やすものか、部門が共同でしか達成できない「目標」を用意するものか。',
             '部門をまたぐ協力は、各部門の評価のどこかで報われるか。',
             '横断プロジェクトに、専任の時間・決める権限・提案の受け取り先は決まっているか。'],
  source='出典：ナデラ『Hit Refresh』／トヨタ自動車75年史／HR総研調査(2021・2026)／テット『サイロ・エフェクト』。失敗事例は複数の実例から再構成した匿名の典型パターン。',
 ),
 dict(
  slug='jirei-mokuhyo', no='No.009', cat='so',
  title='目標設定と目標管理',
  subtitle='対話の道具にした会社と、提出書類で終わった会社',
  win_head='成功：目標を全社公開し、対話でつなぐ（花王、メルカリ）',
  win=['花王は2021年、国内全社員の目標管理をOKRへ。「会社の数字の割り振り」から「本人のこうありたい」起点に反転。',
       '目標は社内サイトで全社公開。同じキーワードの仲間を部門を越えて探せる。',
       'メルカリは全社員の目標を閲覧可能にし、週次チェックインと1on1で期中に話し続ける。',
       '達成度は評価に直結させない。理想は0.7前後——毎回満点なら目標が低すぎるサイン（ドーアのOKR）。'],
  lose_head='失敗：提出書類で終わった目標管理（典型パターン）',
  lose=['期初、締切前日の夜。去年のシートの日付を変えて提出。上司のコメントは「今期も頑張ろう」。',
        '次に開かれるのは期末の評価面談の直前。本人も上司も、目標を思い出すところから始める。',
        '達成率は査定に直結するので、目標は年々少しずつ低く、確実な範囲に。',
        '数年後、「全員が書類を出し、全員がほぼ達成する」精巧な儀式が完成した。'],
  fork='目標を対話の道具にしたか、提出する書類にしたか。',
  questions=['目標が最後に「会話」に登場したのはいつか。期初と期末の2回だけなら提出物管理になっている。',
             '達成率と査定の直結はどのくらい強いか。野心的な目標を守る「査定との距離」を設計できないか。',
             '目標は割り振られたものか、本人が起こしたものか。次の期は「本人のこうありたい」から始めてみる。'],
  source='出典：ニュースイッチ・ITmediaの花王取材記事／SmartHR Mag.のメルカリ取材記事／HRBrain調査(2018)／ドラッカー『現代の経営』。失敗事例は複数の実例から再構成した匿名の典型パターン。',
 ),
 dict(
  slug='jirei-1on1', no='No.011', cat='so',
  title='1on1の定着',
  subtitle='続いた会社と、形だけ残った会社',
  win_head='成功：週1回30分を十年以上続けている（ヤフー）',
  win=['2012年、経営体制の刷新にあわせて導入。上司と部下が週1回・原則30分。当時の社員規模はおよそ6,000人。',
       '目的を「部下のための時間」と定義。進捗確認でも評価面談でもないと最初に言い切った。',
       '制度の号令より先に、管理職へ傾聴とフィードバックのコーチング研修。外部の専門家も交えて自社向けに設計し、改訂を続けた。',
       '全社一斉ではなく人事責任者とその周辺から開始。2017年に書籍化、2025年に増補改訂版。十年以上、同じ枠が動いている。'],
  lose_head='失敗：全社一斉に配って半年で薄れた1on1（典型パターン）',
  lose=['調査結果を受けて翌月から全社実施を決定。配られたのは目的の一枚と進め方マニュアル、実施率の報告フォーマット。',
        '管理職への訓練はなし。「傾聴する」は指示であって技術ではない。20人の部下を持つ課長は面談の枠に追われた。',
        '半年後、実施率だけが報告され中身は誰も見ない状態に。リスケが増え「今週は特になければ飛ばそうか」に。',
        '1年後の調査で「上司と話す機会が足りない」はほとんど動かず。制度は導入済みと記録され、次の施策の検討が始まる。'],
  fork='導入の日ではなく、三ヶ月後の続け方を先に設計したか。',
  questions=['いま用意されているのは、実施率を数える仕組みか、聞く側が育つ仕組みか。',
             '1on1で話したことは評価に使わないと明言され、実際に守られているか。',
             '議題はどちら側が持ち込む設計か。「今週何かある?」から始まる場は主役が入れ替わっている。'],
  source='出典：ダイヤモンド・オンライン(2017/10/4)／本間浩輔『ヤフーの1on1』ダイヤモンド社／リクルートマネジメントソリューションズ「1on1ミーティング導入の実態調査」(2022年1月・n=936)／パーソル総合研究所「部下の成長支援を目的とした1on1ミーティングに関する定量調査」(2024年6月)。失敗事例は複数の実例から再構成した匿名の典型パターン。',
 ),
 dict(
  slug='jirei-omoshiro-jinji', no='No.010', cat='so',
  title='おもしろ人事制度',
  subtitle='思想から生まれた制度と、話題づくりで終わった制度',
  win_head='成功：思想の数だけ制度が生えた会社（サイボウズ、カヤック）',
  win=['サイボウズの離職率は2005年頃28%。「100人100通り」の方針のもと選択型人事・育自分休暇・副業許可などを整え、4%前後に。',
       '制度はどれも「チームワークあふれる会社をつくる」という理念からの演繹。話題づくりではない。',
       '青野社長の言う「制度・ツール・風土の3点セット」。質問責任・説明責任の風土があって自由な制度は回る。',
       'カヤックのサイコロ給は「人の評価を気にしすぎず面白く働く」という思想の表現。「スベった制度」も公開し、続く条件を言語化している。'],
  lose_head='失敗：話題づくりで終わった制度（典型パターン）',
  lose=['採用広報のため、他社事例を集めてユニークな名前の休暇と手当を量産。プレスリリースは話題に。',
        '1年後、目玉の休暇の取得者はほぼゼロ。上司が使っておらず、評価は残業も辞さない働き方に報いたまま。',
        '「いい制度ですよね。使えないけど」が現場の相場観に。管理コストと規程集だけが厚くなる。',
        '制度に惹かれて入った新人が「実際は使えない」と知る。採用のための制度が定着のいちばんの敵になった。'],
  fork='制度を自社の思想から育てたか、他社の話題から借りたか。',
  questions=['その制度は、自社の「何を大事にするか」から説明できるか。他社名でしか説明できないなら根づきにくい。',
             '最初に使うのは誰か。導入日に「上司が使う日」を先に決められないか。',
             '利用率を測っているか。使われなかった制度を「やめる」手続きはあるか。'],
  source='出典：サイボウズ公式（離職率28%→4%）／青野慶久『チームのことだけ、考えた。』／カヤック公式・スタジオパーソル取材記事。失敗事例は複数の実例から再構成した匿名の典型パターン。',
 ),
]
...
SERIES = dict(
  ...
  cases=[
    ...
    ('組', '#9A7A22', 'No.008 縦割りの解消', '分岐点：部門の壁に仕組みの橋を架けたか、交流の場だけを増やしたか'),
    ('組', '#9A7A22', 'No.009 目標設定と目標管理', '分岐点：目標を対話の道具にしたか、提出する書類にしたか'),
    ('組', '#9A7A22', 'No.010 おもしろ人事制度', '分岐点：制度を自社の思想から育てたか、他社の話題から借りたか'),
    ('組', '#9A7A22', 'No.011 1on1の定着', '分岐点：導入の日ではなく、三ヶ月後の続け方を先に設計したか'),
  ],
  coming=['学校統廃合の分かれ道（社会）', '離職が止まった会社、止まらなかった会社（組織）', '住民参加のくじ引き民主主義（社会）'],
)
