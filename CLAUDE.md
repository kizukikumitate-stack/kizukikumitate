...
### カテゴリの棚（右端中央の回遊パネル・2026-07-23 新設）

`data/nav.json` の各ドロップダウンが1つの「棚」になり、`scripts/update-shelf.py` が
カテゴリ内の全ルート直下ページに `<!-- SHELF START/END -->` ブロック（style/script同梱・
自己完結）を生成する。右端中央の縦タブが常時ついてくる**二段階方式**：ホバー可能な端末は
カーソルを合わせると全リストが展開、タッチ端末はタップで開閉（常時展開は本文に重なり
読みにくいため不採用・2026-07-23 森本さん確定）。
...
- **セーフゾーンを守ること**（2026-08 修正）。SNS・チャットのリンクプレビューは
  1200×630 を 1.3〜1.4:1 に**中央トリミング**して表示するため、左右が最大 190px ほど
  削られる。文字・バー・ブランド表記はすべて **x=190〜1010px の内側**に収める。
  `ogp-template.html` の `.pad` 左右パディング 158px（+`inset:32px` = 190px）がその境界で、
  ここを狭めると端の文字が切れる。専用デザイン（`_bespoke`）を手で作るときも同じ範囲を守る
